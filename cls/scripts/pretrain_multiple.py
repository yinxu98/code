...
from cls.tools import (AverageMeter, ProgressMeter, TimeMeter,
                       ...
                       build_criterion_sim, build_optimizer, log,
                       save_checkpoint)
...
def train(cfg):
    ...
    for epoch in range(cfg.runner.epoch):
        losses = {
            'aet': AverageMeter('loss_aet', ':.4f'),
            'sim': AverageMeter('loss_sim', ':.4f'),
            'all': AverageMeter('loss_all', ':.4f')
        }
        progress = ProgressMeter(
            cfg.runner.epoch,
            [eta, losses['aet'], losses['sim'], losses['all']],
            prefix='Pretrain: ',
        )
        ...
        for imgs, gt_param in data_loader:
            ...
            # compute output
            p1, p2, r1, r2, pred_param = model(imgs[0], imgs[1])
            ...
            loss_all = loss_aet + cfg.model.loss_scale * loss_sim

            losses['aet'].update(loss_aet.item(), imgs[0].size(0))
            losses['sim'].update(loss_sim.item(), imgs[0].size(0))
            losses['all'].update(loss_all.item(), imgs[0].size(0))
